...
public class TransformerInputInterceptor implements ChannelInterceptor {

  @Override
  public Message<?> preSend(@NotNull Message<?> message, @NotNull MessageChannel channel) {
    LOG.info("[IN]-> preSend '{}' to '{}'", message.getHeaders().get("id"), channel);
    return ChannelInterceptor.super.preSend(message, channel);
  }

  @Override
  public void postSend(@NotNull Message<?> message, @NotNull MessageChannel channel, boolean sent) {
    LOG.info("[IN]-> postSend '{}' to '{}', sent={}", message.getHeaders().get("id"), channel, sent);
  ...
  public void afterSendCompletion(
      ...
      boolean sent,
      Exception ex) {
    LOG.info("[IN]-> afterSendCompletion '{}' to '{}', sent={}, ex='{}'",
    ...
    if (sent) {
      Optional.ofNullable((Checkpointer) message.getHeaders().get(CHECKPOINTER))
          .map(c -> c.success()
              .doOnSuccess(s -> LOG.info("[IN]-> Message successfully checkpointed: '{}'", message.getHeaders().get("id")))
              .doOnError(error -> LOG.error("[IN]-> Checkpoint failed", error))
              .subscribe());
    } else {
